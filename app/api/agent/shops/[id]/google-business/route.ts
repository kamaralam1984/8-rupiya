...
/**
 * POST /api/agent/shops/[id]/google-business
...
/**
 * GET /api/agent/shops/[id]/google-business
...
export async function GET(
  ...
  try {
    ...
    // Find the shop
    const shop = await AgentShop.findById(id).select('googleBusinessAccount agentId');
    if (!shop) {
      return NextResponse.json(
        { success: false, error: 'Shop not found' },
        { status: 404 }
