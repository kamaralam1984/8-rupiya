    ...
    const fetchData = async () => {
      try {
        ...
        if (data?.success && data?.shops) {
          ...
          const topRated = (fallbackData.businesses || [])
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4">
            ...
            return (
                <Link key={biz.id} href={`/shop/${biz.id}`}>
                  <article className="group rounded-xl bg-white shadow-md border border-gray-200 overflow-hidden transition-all duration-300 hover:shadow-lg hover:border-yellow-300 cursor-pointer">
                <div className="relative h-40 sm:h-48 overflow-hidden">
                  ...
                  />
                      {index < 3 && !isSearchActive && (
                        <span className="absolute top-2 right-2 z-20 inline-flex items-center px-2 py-1 rounded-md text-xs font-bold bg-yellow-500 text-white shadow-md">
                          ⭐ Top Rated
                    </span>
                  )}
                </div>
